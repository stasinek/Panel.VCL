...
if (KluczRejestuSystemuWindows->ValueExists("clientrect"))
   {KluczRejestuSystemuWindows->ReadBinaryData("clientrect",&options.clientrect, sizeof(RECT));
    if (options.clientrect.right-options.clientrect.left != 400 || options.clientrect.bottom-options.clientrect.top != 30)
        {options.clientrect.right = 400;
...
Ruszacz = new ssTSoft_::WindowsMover();
...
else Ruszacz->MouseDown(this->Handle,Button,X,Y);
}
//---------------------------------------------------------------------------
...
Ruszacz->MouseMove(this->Handle,true,X,Y,&formRect,50,(formRect.top+formRect.bottom)/2);
}
//---------------------------------------------------------------------------
...
void __fastcall TMain_form::FormMouseUp(TObject*,
		TMouseButton Button, TShiftState, int X, int Y)
{
Ruszacz->MouseUp(this->Handle,Button,X,Y);
updateing = 0;
}
//---------------------------------------------------------------------------
...
{
if ((GetAsyncKeyState(4) & 0x8000)==0 && (Pressed.fCtrl==true && Pressed.fAlt==true)==0)
	return;
...
{
Zeus_form->Visible = SpeedButton1->Down;
}
//---------------------------------------------------------------------------
